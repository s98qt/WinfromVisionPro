    ...
    public class WorkflowService
    {
                ...
                if (_currentTemplate != null)
                {
                    ...
                    await Task.Run(async () => {
                        ...
                        if (batch.Count > 0) // 单次检测
                        {
                            ...
                            ExecuteStandardCheckAsync(batch[0]);
                            // 动态计算批处理超时时间
        ...
        private async Task ExecuteArLoopAsync(WorkStep step)
        {
            ...
            try
            {
                ...
                // 等待相机，确保 _latestFrame 有数据
                //int waitCount = 0;
                //while (camera.GetCogImage() == null && waitCount < 10) // 假设你有方法获取 _latestFrame 或通过 ImageCaptured 事件判断
                //{
