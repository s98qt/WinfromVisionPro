    ...
    public class YoloOBBInference : IDisposable
    {
        ...
        private void ResolveInputSizeFromModel()
        {
            try
            {
                ...
                {
                    _targetSize = dims[2];
                    LoggerService.Info($"[OBB模型加载] 自动识别输入尺寸: {dims[2]}x{dims[3]}");
                }
                else
                {
                    LoggerService.Info($"[OBB模型加载] 输入维度为动态或不明确，沿用默认尺寸: {_targetSize}");
                }
            }
            catch (Exception ex)
            {
                LoggerService.Info($"[OBB模型加载] 解析输入尺寸失败，使用默认值 {_targetSize}: {ex.Message}");
            }
        }
        ...
        public List<YoloOBBPrediction> Predict(Bitmap image, float confThreshold = 0.25f, float iouThreshold = 0.45f)
        {
            ...
            LoggerService.Info($"[OBB推理] 原始图像尺寸: {image.Width}x{image.Height}");
        ...
        private List<YoloOBBPrediction> PostprocessSimple(Tensor<float> output, int originalW, int originalH, float ratio, int padLeft, int padTop, float confThreshold, float iouThreshold)
        {
            ...
            LoggerService.Info($"[OBB简化后处理] 通过置信度阈值: {filteredData.Count}/{rows}");
            ...
            {
                LoggerService.Info($"[坐标还原] 模型输出: cx={data.x:F2}, cy={data.y:F2}, w={data.w:F2}, h={data.h:F2}, angle={data.angle:F4}");
                ...
                LoggerService.Info($"[坐标还原] 还原后: cx={x:F2}, cy={y:F2}, w={w:F2}, h={h:F2}, angle={angle:F4} (ratio={ratio:F4}, padLeft={padLeft}, padTop={padTop})");

                // 计算旋转矩形的4个角点
                PointF[] corners = CalculateRotatedCorners(x, y, w, h, angle);

                LoggerService.Info($"[坐标还原] 角点: P1=({corners[0].X:F2},{corners[0].Y:F2}), P2=({corners[1].X:F2},{corners[1].Y:F2}), P3=({corners[2].X:F2},{corners[2].Y:F2}), P4=({corners[3].X:F2},{corners[3].Y:F2})");
